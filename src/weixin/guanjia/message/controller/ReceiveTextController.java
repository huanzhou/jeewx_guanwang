...
public class ReceiveTextController {
	...
	private static final Logger LOG = LoggerFactory.getLogger(ReceiveTextController.class);
	...
	public AjaxJson deleteSmsGroup(ReceiveText receiveText, HttpServletRequest req) {
		...
		receiveText = (ReceiveText) this.systemService.getEntity(ReceiveText.class, receiveText.getId());
	...
	public AjaxJson updateAndSave(ReceiveText receiveText, HttpServletRequest req) {
		AjaxJson j = new AjaxJson();
		ReceiveText tempTextMessage = (ReceiveText) this.systemService
				.getEntity(ReceiveText.class, receiveText.getId());
		this.message = "回复信息成功！";
		try {
			...
			if (LOG.isInfoEnabled()) LOG.info("......jsonObj..." + jsonObj.toString());

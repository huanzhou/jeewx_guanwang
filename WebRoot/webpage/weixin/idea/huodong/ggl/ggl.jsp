...
<script type="text/javascript">
	$(document).ready(function(){
		var picPath = "plug-in/weixin/images/ggl/nothing.png";
		 var prize = "${prize}";
		 var status=0;
		 $("#ul_id").find("input[name='hiddenawardsname']").each(function(){
			 var t = $(this).attr("title");
			 if(prize == t){
				 status =1;
				 var name = $(this).val();
				 picPath = "awardsImage.do?awardsname="+name;
			 }
		 });
		if(status==0){
		...
		$("#wScratchPad2").wScratchPad({
                		...
                		var s =0;
                		 $("#ul_id").find("input[name='hiddenawardsname']").each(function(){
                			 ...
                			 if(prize == tt){
                				...
                				message = "恭喜你,获得"+names+"！";
                				$("#prizetype").html(names);
                				$("#hidden_awards").val(ids);
         ...
         $("#save-btn").bind("click",function(){
			...
			$.ajax({
           		...
           		method:"POST",
           		dataType:"JSON",
           		data:{mobile:tel,username:name,address:adr,awardsid:awards},
           		success:function(data){
           		   if(data.success){
           		   		alert("提交成功，谢谢参与！");
           		   }else{
...
<input type="hidden" id="hidden_awards" id="hidden_awards" />
...
<input class="pxbtn" id="save-btn" name="提 交" type="button" value="提 交">
	...
	<ul id="ul_id"> 
			<c:forEach items="${awardslist}" var="awardsmodel" varStatus="status">
			<input type="hidden" id="${awardsmodel.awardslevle.id}" name="hiddenawardsname" title="${status.count}" value="${awardsmodel.awardslevle.awardsName}" />
			
				<li>${awardsmodel.awardslevle.awardsName}：
					<c:forEach items="${awardmap}" var="m">
					<c:if test="${m.key==awardsmodel.id}">
					  <c:forEach items="${m.value}" var="item">
					    <c:out value="${item.awardmodel.name}" />
					  </c:forEach>
					 </c:if>
					</c:forEach>
				，数量：${awardsmodel.amount}
				</li>
			</c:forEach>
	</ul>
	</div>

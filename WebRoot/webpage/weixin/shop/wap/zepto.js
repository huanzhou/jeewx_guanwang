...
utils.namespace("mobile.o2ocn.addrList"), utils.namespace("mobile.o2ocn.addrchoose"), function () {
    ...
    g.init = function () {
        ...
        if (1 > d && (d = 0), d += b.reduce, 1 > d && (d = 0), a = d + (b.isFreeFee ? 0 : b.curShipFee), "1" === c.payType && 500 > a && utils.showBubble("您购买的商品应付总价低于5元，无法使用货到付款功能，请用在线付款下单!"), "1" === c.payType) {
            var e = b.divPrice(a);
        ...
        if (!a.hasClass("btn_disabled")) {
            ...
            $.each(window.subParam, function (a, b) {
                ...
                b.errCode ? 3 == b.errCode ? (a.removeClass("btn_disabled"), utils.showBubble(b.errMsg)) : (a.removeClass("btn_disabled"), utils.showAjaxErr(b, "下单失败，请重试")) : utils.payDeal(b, a)
            })
        }
...
utils.namespace("mobile.o2ocn.addrList"), utils.namespace("mobile.o2ocn.addrchoose"), function () {
    ...
    g.init = function () {
        ...
        if (b.singleCheckedNode) {
            ...
            if (b.usetype3 && b.typevalue3) {
                if (b.typevalue3*100 > sp) {
                 
                    utils.showBubble("积分金额大于总价格!");
                }
            ...
            if (b.usetype4 && b.typevalue4) {
                if (b.typevalue4 > sp) {
                   // utils.showBubble("使用金额大于总价格");
                    utils.showBubble("使用金额大于总价格!"), void 0;
                }
                bbb = parseFloat(b.typevalue4 * 100);
                    sp =Math.round(sp) - parseFloat(b.typevalue4 * 100);
		    sp==1?sp=0:sp=sp;
		sp=((sp / 100).toFixed(2)==-0.00?0.00:(sp / 100).toFixed(2))*100;
	        aaa=sp;
            }
        }
                ...
                if (b.usetype3) {
                    if (!b.typevalue3) {
                        ...
                        if (b.realPrice && b.realPrice < 0) {
                            return utils.showBubble("积分优惠金额不能大于实付金额!"), void 0;
                        }
                    }
                }
                if (b.usetype4) {
        ...
        //重置到全部未选中
        $("span[name='singleChoice']").removeClass("checked");
        this.usetype1 = false;
        this.usetype2 = false;
        ...
        var b = this;
        if (b.singleCheckedNode) {
            var radiobtn = $(a).closest("dl").find("span[name='singleChoice']");
            var radio = radiobtn[0];
            if (radio.tag == 1) { radio.tag = 0; radiobtn.removeClass("checked"); }
            else {
                radio.tag = 1;
                radiobtn.addClass("checked");
            }
            var singleType = radiobtn.attr("value");
            if (singleType == 2) {
                if (radio.tag == 0) {
                    document.getElementById("couponid1").style.display = "none";
                }
                else {
                    document.getElementById("couponid1").style.display = "block";
                }
            }
            if (radio.tag == 1) {
                //使用会员
                ...
                if (singleType == 1) {
                    ...
                    var vId = document.getElementById("hidSwitch").tag;
                    if (vId.indexOf("2") > 0) {
                        document.getElementById("couponid1").style.display = "none";
                        document.getElementById("singleChoice2").tag = 0;
                    }
                    if (vId.indexOf("3") > 0) {
                        document.getElementById("singleChoice3").tag = 0;
                    }
                    if (vId.indexOf("4") > 0) {
                        document.getElementById("singleChoice4").tag = 0;
                    }
                    ...
                    var vId = document.getElementById("hidSwitch").tag;
                    if (vId.indexOf("1") > 0) {
                        document.getElementById("singleChoice1").tag = 0;
                    }
                    if (vId.indexOf("3") > 0) {
                        document.getElementById("singleChoice3").tag = 0;
                    }
        ...
        var b = this;
        var checkboxbtn = $(a).closest("dl").find("span[name='multiChoice']");
        if (!checkboxbtn.hasClass("checked")) {
            this.usevip = true;
            this.usetype1 = true;
            this.typevalue1 = $(a).closest("dl").find("input[name='multiChoiceValue']").val();
            checkboxbtn.addClass("checked");
        } else {
            this.usetype1 = false;
            a && b.countPrice();
            ...
            checkboxbtn.removeClass("checked");
        }
        a && b.countPrice();
        ...
        if (!checkboxbtn.hasClass("checked")) {
            ...
            this.typevalue2 = false;
            $("#couponId11").hide();
            $(a).closest("dl").find("span[name='multiChoiceValue']").removeClass("checked");
            if (!b.multiCheckedNode) {
                this.usevip = b.usevip || false;
            }
            checkboxbtn.removeClass("checked");
        ...
        //使用单一代金券
        var b = this;
        var radiobtn = $(a).find("span[name='singleChoiceValue']");
        if (radiobtn.length == 0) {
            radiobtn = $(a).find("span[name='multiChoiceValue']");
            if (radiobtn.hasClass("checked")) {
                var items = $(a).closest("dl").find("span[name='multiChoiceValue'].checked").not(radiobtn).removeClass("checked");
            }
            else {
                $(a).closest("dl").find("span[name='multiChoiceValue'].checked").removeClass("checked");
            }
        }
        else {
            if (radiobtn.hasClass("checked")) {
                var items = $(a).closest("dl").find("span[name='singleChoiceValue'].checked").not(radiobtn).removeClass("checked");
            }
            else {
                $(a).closest("dl").find("span[name='singleChoiceValue'].checked").removeClass("checked");
            }
        }
        if (!radiobtn.hasClass("checked")) {
            radiobtn.addClass("checked");
            //使用代金券
            this.typevalue2 = radiobtn.attr("value");
            this.djqmz = radiobtn.next().val();
            this.djqje = radiobtn.next().next().val();
            this.djqmc = radiobtn.next().next().next().val();
            a && b.countPrice();
        } else {
            this.typevalue2 = false;
            a && b.countPrice();
        ...
        if ($(a).closest("dl").find("input[name='singleChoiceValue']").length > 0) {
            ...
            var maxValue = $(a).closest("dl").find("input[name='singleChoiceValue']").attr("maxvalue");
            if (parseFloat(curValue) > parseFloat(maxValue)) {
                utils.showBubble("当前可用积分兑换金额是:" + maxValue);
                this.typevalue3 = maxValue;
                b.countPrice();
              
            }
            else {
                this.typevalue3 = curValue;
                if (b.singleCheckedNode && b.usetype3) {
                    b.countPrice();
                }
                //utils.showBubble(this.typevalue4);
            }
    ...
    g.init = function () {
        ...
        b.payPrice = a, b.priceNode.html("&yen;" + (a / 100).toFixed(2)), def = (a / 100).toFixed(2), b.reduce + b.couponReduce < 0 ? ($("#dealoff-div").removeClass("qb_none"), $("#dealoff-price").html("&yen;" + ((-1 * b.reduce + -1 * b.couponReduce) / 100).toFixed(2))) : $("#dealoff-div").addClass("qb_none")
    }, g.divPrice = function (a) {
        var b = {
            price: (a / 100).toFixed(2),
            free: "0.00",
    ...
    a.init = function () {
        ...
        utils.ajaxReq({
            ...
            //b.errCode ? 260 == b.errCode ? utils.showBubble("登录超时") : utils.showAjaxErr(b, "删除失败，请重试。") : (utils.showBubble("删除订单成功"), 
            $(a).closest("div.mod_cell").addClass("qb_none"),
            location.href = window.basePath + "/my/index.html?index=1&msgCode=" + b.errMsg + "&" + window.baseParam
        }, function () {
            utils.showBubble("请求遇到了点问题，请重试。")
        })
    }, a.confirmReceive = function (a) {
        utils.ajaxReq({
           ...
           // b.errCode ? 260 == b.errCode ? utils.showBubble("登录超时") : utils.showAjaxErr(b, "确认收货失败，请重试。") : (utils.showBubble("确认收货成功"), $(a).addClass("qb_none")),
            utils.showBubble("确认收货成功"), $(a).addClass("qb_none")
            location.href = window.basePath + "/my/index.html?index=2&msgCode=" + b.errMsg + "&" + window.baseParam
        }, function () {
            utils.showBubble("请求遇到了点问题，请重试。")
        })
    }, a.changePay = function (a) {

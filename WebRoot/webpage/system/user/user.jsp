     ...
     ${user.userName }
		...
		<tr>
			...
			<select id="type" name="type" >
				<option value="2" <c:if test="${user.type==2}">selected="selected"</c:if>>商家用户</option>
				<option value="1" <c:if test="${user.type==1}">selected="selected"</c:if>>普通用户</option>
				<option value="3" <c:if test="${user.type==3}">selected="selected"</c:if>>管理员</option>
			</select>
			<span class="Validform_checktip">用户类型</span>
			</td>
		</tr>
		<tr>
